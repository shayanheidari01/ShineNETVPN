...
async fn fetch_token_with_email_code(settings: &TeamSettings, email: &str) -> Result<String> {
    ...
    let code = prompt_login_code(email, 1).await?;
    ...
    for attempt in 1..=CODE_ATTEMPTS {
        let code = match attempt {
            1 => code.clone(),
            _ => prompt_login_code(email, attempt).await?,
        };

        let confirmed = client
            .post(&callback)
            .form(&[("code", code.as_str()), ("nonce", nonce.as_str())])
            .send()
            .await
        ...
        if let Some(token) = extract_jwt_from_html(&body) {
            log::info!("[+] signed in to team {} with the email code", settings.team);
            return Ok(token);
        }
