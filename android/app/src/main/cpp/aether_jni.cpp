...
Java_com_shythonx_shinenet_1vpn_AetherNativeCore_nativePrepare(JNIEnv* env, jobject, jstring config) {
...
Java_com_shythonx_shinenet_1vpn_AetherNativeCore_nativeLastResult(JNIEnv* env, jobject) {
    return last_string(env, aether_last_result());
}

extern "C" JNIEXPORT jint JNICALL
Java_com_shythonx_shinenet_1vpn_AetherNativeCore_nativeStart(JNIEnv* env, jobject, jstring config, jint tun_fd) {
    const auto config_text = copy_jstring(env, config);
    if (config_text.empty()) return -1;
...
extern "C" JNIEXPORT jint JNICALL
Java_com_shythonx_shinenet_1vpn_AetherNativeCore_nativeStop(JNIEnv*, jobject) {
    return aether_stop();
}

extern "C" JNIEXPORT jboolean JNICALL
Java_com_shythonx_shinenet_1vpn_AetherNativeCore_nativeIsRunning(JNIEnv*, jobject) {
    return aether_is_running() != 0;
}

extern "C" JNIEXPORT jboolean JNICALL
Java_com_shythonx_shinenet_1vpn_AetherNativeCore_nativeIsReady(JNIEnv*, jobject) {
    return aether_is_ready() != 0;
}


extern "C" JNIEXPORT jstring JNICALL
Java_com_shythonx_shinenet_1vpn_AetherNativeCore_nativeLastError(JNIEnv* env, jobject) {
    return last_string(env, aether_last_error());
}

extern "C" JNIEXPORT jstring JNICALL
Java_com_shythonx_shinenet_1vpn_AetherNativeCore_nativeLastLog(JNIEnv* env, jobject) {
    return last_string(env, aether_last_log());
}

extern "C" JNIEXPORT void JNICALL
Java_com_shythonx_shinenet_1vpn_AetherNativeCore_nativeAttach(JNIEnv* env, jobject, jobject service) {
    std::lock_guard<std::mutex> lock(g_service_mutex);
...
extern "C" JNIEXPORT void JNICALL
Java_com_shythonx_shinenet_1vpn_AetherNativeCore_nativeDetach(JNIEnv* env, jobject) {
    std::lock_guard<std::mutex> lock(g_service_mutex);
