...
class IntelligentServerSelector {
  static final IntelligentServerSelector _instance = IntelligentServerSelector._internal();
  factory IntelligentServerSelector() => _instance;
  IntelligentServerSelector._internal();

  final ServerOptimizationService _serverOptimization = ServerOptimizationService();
  final ConnectionOptimizationService _connectionOptimization = ConnectionOptimizationService();
  ...
  Future<String?> getBestServer({
    ...
    try {
      ...
      final filteredServers = await _applyAdvancedFiltering(servers, onStatusUpdate);
      ...
      throw Exception('Failed to connect to any server after intelligent selection');
  ...
  Future<List<String>> _applyAdvancedFiltering(List<String> servers, Function(String)? onStatusUpdate) async {
    if (servers.length <= 8) return servers;
    ...
    for (final server in servers) {
      ...
      if (performance != null) {
        ...
        final responseTimePenalty = log(performance.avgResponseTime.clamp(1, 5000)) * 8.0;
        score -= responseTimePenalty;
        
        // Recent activity bonus (servers used in last hour get priority)
        final hoursSinceLastUse = currentTime.difference(performance.lastUsed).inHours;
    ...
    onStatusUpdate?.call('🎯 Selected ${selectedServers.length} optimal servers');
  ...
  double _calculateAdvancedPatternScore(String server) {
    ...
    for (final pattern in recentPatterns.where((p) => p.server == server && p.successful)) {
      final hoursAgo = currentTime.difference(pattern.timestamp).inHours;
      final timeWeight = exp(-hoursAgo / 24.0); // Exponential decay over 24 hours
  ...
  String? _getCachedPrediction() {
    final bestPrediction = _serverPredictions.values
        .where((p) => DateTime.now().difference(p.timestamp) < _predictionCacheTimeout)
  ...
  void _recordSuccessfulSelection(String server) {
    ...
    if (_connectionPatterns.length > _maxPredictionHistory) {
      _connectionPatterns.removeRange(0, _connectionPatterns.length - _maxPredictionHistory);
    }
  ...
  void recordFailedSelection(String server, int responseTime) {
    ...
    metrics.avgResponseTime = metrics.totalResponseTime / metrics.totalConnections;
  ...
  void _updatePerformanceMetrics() {
    for (final server in _performanceHistory.keys) {
      ...
      if (metrics.totalConnections > 0) {
        metrics.successRate = metrics.successfulConnections / metrics.totalConnections;
      }
  ...
  Future<void> _loadLearningData() async {
    try {
      ...
      final performanceData = prefs.getString('intelligent_performance_history');
  ...
  Future<void> _saveLearningData() async {
    try {
      ...
      final performanceData = _performanceHistory.map(
        (key, value) => MapEntry(key, value.toJson())
      );
      await prefs.setString('intelligent_performance_history', json.encode(performanceData));
      
      // Save connection patterns (keep only recent ones)
      final recentPatterns = _connectionPatterns
          .where((p) => DateTime.now().difference(p.timestamp) < Duration(days: 30))
          .toList();
      final patternsData = recentPatterns.map((p) => p.toJson()).toList();
      await prefs.setString('intelligent_connection_patterns', json.encode(patternsData));
      
      // Save adaptive metrics
      final adaptiveData = _adaptiveMetrics.map(
        (key, value) => MapEntry(key, value.toJson())
      );
      await prefs.setString('intelligent_adaptive_metrics', json.encode(adaptiveData));
  ...
  Map<String, dynamic> getPerformanceStats() {
    ...
    final avgSuccessRate = _performanceHistory.values
        .map((m) => m.successRate)
        .fold(0.0, (a, b) => a + b) / max(totalServers, 1);
  ...
  bool _shouldUseAdaptiveSelection() {
    ...
    final timeSinceLastSelection = DateTime.now().difference(_lastAutoSelection!);
  ...
  Future<String?> _getAdaptiveServer(Function(String)? onStatusUpdate) async {
    ...
    try {
      ...
      final testResult = await _serverOptimization.testServerConnection(_currentBestServer!);
      
      if (testResult.success && testResult.responseTime < 2000) {
        _updateAdaptiveMetrics(_currentBestServer!, true, testResult.responseTime);
        return _currentBestServer;
      } else {
        _updateAdaptiveMetrics(_currentBestServer!, false, testResult.responseTime);
  ...
  Future<String?> _handleEmptyServerList(Function(String)? onStatusUpdate) async {
    onStatusUpdate?.call('⚠️ No servers available, using fallback...');
    
    // Try to get cached servers from optimization service
    try {
      final cachedServers = await _serverOptimization.getOptimizedServerList(forceRefresh: false);
  ...
  Future<String?> _handleSelectionFailure(Function(String)? onStatusUpdate) async {
  ...
  void _updateAdaptiveMetrics(String server, bool success, [int? responseTime]) {
  ...
  List<String> _selectDiverseServers(List<ScoredServer> scoredServers) {
    if (scoredServers.length <= 12) {
      return scoredServers.map((s) => s.server).toList();
    }
    
    // Take top performers and add some diversity
    final topServers = scoredServers.take(8).map((s) => s.server).toList();
    final remainingServers = scoredServers.skip(8).take(4).map((s) => s.server).toList();
    
    return [...topServers, ...remainingServers];
  }
  ...
  void _updateAllAdaptiveMetrics() {
    ...
    for (final metrics in _adaptiveMetrics.values) {
      final daysSinceUpdate = currentTime.difference(metrics.lastAdaptiveUpdate).inDays;
  ...
  void dispose() {
    ...
    _connectionOptimization.dispose();
    _saveLearningData();
  }
}
...
class PerformanceMetrics {
  ...
  factory PerformanceMetrics.fromJson(Map<String, dynamic> json) {
    ...
    metrics.lastUsed = DateTime.parse(json['lastUsed'] ?? DateTime.now().toIso8601String());
    return metrics;
  }
}
...
class AdaptiveServerMetrics {
  ...
  double get recentSuccessRate => recentConnections > 0 ? recentSuccesses / recentConnections : 0.0;
  ...
  void updateMetrics(bool success, int responseTime) {
    ...
    if (recentResponseTimes.length >= 3) {
      ...
      final variance = recentResponseTimes
          ...
          .reduce((a, b) => a + b) / recentResponseTimes.length;
      final standardDeviation = sqrt(variance);
      
      // Lower standard deviation = higher consistency
      consistencyRating = 1.0 - (standardDeviation / (mean + 1)).clamp(0.0, 1.0);
    }
  ...
  factory AdaptiveServerMetrics.fromJson(Map<String, dynamic> json) {
    ...
    metrics.lastAdaptiveUpdate = DateTime.parse(json['lastAdaptiveUpdate'] ?? DateTime.now().toIso8601String());
